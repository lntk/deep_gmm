...
class GMMNet(nn.Module):
    ...
    def init_gmm(self, shape, device):
        ...
        """
        EXP 2.4: Constant mean
        """
        # mu = torch.cat([torch.ones(B, C, H, W).float().to(device) * 60 * (i + 1) for i in range(K)], dim=1) / 255.
...
class GMMBlock(nn.Module):
    def __init__(self, in_channels, num_component):
        ...
        EXP 2.1
        """
        self.f_simple = self.nonlinear_f(layers=[in_channels, 16, 8, 4, 1], last_activation="sigmoid")
