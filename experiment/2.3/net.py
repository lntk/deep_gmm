...
class GMMBlock(nn.Module):
    def __init__(self, in_channels, num_component):
        ...
        self.f_pi    = self.nonlinear_f(layers=[2 * num_component, 4, num_component], last_activation="softmax")        
        self.f_mu    = self.nonlinear_f(layers=[in_channels + in_channels * num_component + num_component, 8, in_channels * num_component], last_activation="sigmoid")
        self.f_sigma = self.nonlinear_f(layers=[in_channels + in_channels * num_component + num_component, 4, num_component], last_activation="sigmoid")        
                
        self.f_tau   = self.nonlinear_f(layers=[num_component, 4, 1])
