...
class GMMNet(nn.Module):
    def __init__(self, in_channels, num_component, blocks=(8, 16, 32)):        
        ...
        self.conv_decoder = nn.Sequential(
            nn.Conv2d(in_channels=self.blocks[0] * 2, out_channels=1, kernel_size=1, stride=1, padding=0, bias=True),            
            nn.Sigmoid()
        )     
        ...
        self.upsamples = nn.ModuleList(
            [
                Upsample(in_channels=self.blocks[i] * 2, out_channels=self.blocks[i - 1]) 
    ...
    def forward(self, frames, targets):
        ...
        for i in range(sequence_length):
            ...
            encode_1_map = self.upsamples[1](torch.cat((encode_2_map * encode_2, encode_2), dim=1))
            assert encode_1_map.shape == torch.Size(np.array([B, 1, int(H/2), int(W/2)]))            
            
            encode_0_map = self.upsamples[0](torch.cat((encode_1_map * encode_1, encode_1), dim=1))
            assert encode_0_map.shape == torch.Size(np.array([B, 1, H, W]))                                                                      
            
            output = self.conv_decoder(torch.cat((encode_0_map * encode_0, encode_0), dim=1))
